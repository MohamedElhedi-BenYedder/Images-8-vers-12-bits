...
#%% SECTION 2 Definition des chemins
path8bits ="/cal/exterieurs/benyedder-21/ProjetIMA/8-bits-vers-16-bits/8-bits/"
path16bits="/cal/exterieurs/benyedder-21/ProjetIMA/8-bits-vers-16-bits/16-bits/"
#%% SECTION 3 Fonctions usuelles
def Decompress(im,inNbits,outNbits,r=None):
    ...
    """
    step = 2**(outNbits-inNbits)
    decompressedImg = im.copy()
    if(isinstance(r,int)):
        r = r%step
    if (r==None):
        SIZE = decompressedImg.shape
       # r = np.random.random_integers(low=0,high=step-1,size=SIZE)
    decompressedImg = decompressedImg * step + r
...
def var_image(im,x0=None,y0=None,x1=None,y1=None):
...
#%% Section 4
listOf8Bits = os.listdir(path8bits)
listOf16Bits = os.listdir(path16bits)

Images8 =  [skio.imread(path8bits+k) for k in listOf8Bits]
Images16 =  [skio.imread(path16bits+k) for k in listOf16Bits]
Images16From8 = [Decompress(k,8,16,0) for k in Images8]
for im16 in Images16 :
    viewimage(im16,MINI=0.0,MAXI=2**16-1)
for im16 in Images16From8 :
    viewimage(im16,MINI=0.0,MAXI=2**16-1)
